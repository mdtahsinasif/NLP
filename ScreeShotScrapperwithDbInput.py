...
def clickPics():
    ...
    try:
       # last_hour_date_time = datetime.now() - datetime.timedelta(hours=24 * 365)
        # query = "{pub_date:ISODate('+last_hour_date_time+')}"
        print("clickPics Entry")
        mydoc = urlCollection.find()
        print("mydoc=========>",mydoc)


        # mydoc = urlCollection.find({
        #         'pub_date': {'$lte': datetime.now() - datetime.timedelta(hours=24)}
        #     })
    except:
        print("Hello Bang There is an Error in URL DB connectivity")
    for row in mydoc:
...
if __name__ == "__main__":
    ...
    urlCollection = db.ce_mail_template_data
