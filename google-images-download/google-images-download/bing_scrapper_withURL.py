...
def user_input():
    ...
    if object_check['config_file'] != '':
        ...
        #                     help='time range for the age of the image. should be in the format {"time_min":"MM/DD/YYYY","time_max":"MM/DD/YYYY"}',
        #                     type=str, required=False)
       # args.time_range = "07/07/2019"
        args.limit = 20
        args.search = 'spear phising mail'
        args.download = True
        #args.time = 'past-year'
...
class googleimagesdownload:
    ...
    def download_extended_page(self, url, chromedriver):
        ...
        try:
            # chrome driver input -- cyfirma
            browser = webdriver.Chrome("C:\\Users\\TahsinAsif\\Downloads\\chromedriver.exe", options=options)
        except Exception as e:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    output_file.write(data)
                  #  print(image_name+"::::->",data,"image_url:::->",image_url)

                    imageList.append(image_name)
                    image_urlList.append(str(image_url).casefold())
                    inerDict[image_name] = image_url
                    output_file.close()

                    # Cyfirma Changing save_source to True to save in url in folder
                    save_source = False
                    if save_source:
                        #Cyfirma Changing save_source to ImageUrlContainer
                        list_path = main_directory + "/" + "ImageUrlContainer" + ".txt"
        ...
        while count < limit + 1:
            ...
            if object == "no_links":
                ...
                if download_status == "success":
                    ...
                    #print("return_image_name==========", object['image_link'])
                    #Cyfirma input
                    #print("return_image_name==========", object)
            ...
            for sky in suffix_keywords:  # 2.for every suffix keywords
                ...
                while i < len(search_keyword):  # 3.for every main keyword
                    ...
                    if arguments['image_directory']:
                       ...
                       # dir_name = 'C:\Backup\PycharmProjects\PycharmProjects\google-images-download\google-images-download\images'
                    else:
                        dir_name = search_term + (
                            '-' + arguments['color'] if arguments['color'] else '')  # sub-directory
                    #    dir_name = 'C:\Backup\PycharmProjects\PycharmProjects\google-images-download\google-images-download\images\img'
...
def dbUpdate():
    ...
    while True:
        ...
        if os.path.isdir("C:\Backup\PycharmProjects\PycharmProjects\google-images-download\google-images-download\images\spear_phising_mail"):
    ...
    for file in files:
        try:
            dw={}
           # print("image Name::->", file.title())
            img = cv2.imread(file)
            text = pytesseract.image_to_string(img)
            subs =str(file.title())
            my_new_string = re.sub('Images/Spear_Phising_Mail', '', subs)
            new_string =(my_new_string.split('.')[1])
            for key,value in inerDict.items():
                key = (key.split('.')[0])
           #     print("new_string.lower::->",new_string.lower(),"key::->",key)
                if str(key).lower() == new_string.lower():
                     print("Match Present:::",value)
                     res = value
            print(new_string, "::::->", res, "::::->")
                #print()

            query = {"url": res}
            newvalues = {"$set": {"content": text}}
...
if __name__ == "__main__":
    img_dir = "images/spear_phising_mail"  # Enter Directory of all images
